...
Install or update Bitwarden password manager on Manjaro Linux.
...
DESCRIPTION:
    This script will:
    1. Check if Bitwarden is already installed
...
update_bitwarden() {
    ...
    if [[ "$dry_run" == "true" ]]; then
        ...
        $aur_helper -S --noconfirm "$PACKAGE_NAME"
        log "Bitwarden updated successfully"
    fi
}
...
main() {
    ...
    if is_package_installed; then
        ...
        log "Bitwarden is already installed (version: $installed_version)"
        
        if [[ "$force" == "true" ]]; then
            log "Force flag specified, reinstalling..."
            update_bitwarden "$aur_helper" "$dry_run"
            return 0
        fi
        ...
        case $cmp_result in
            0)
                log "Bitwarden is up to date (version: $installed_version)"
                ;;
            1)
        ...
        log "Bitwarden is not installed"
        install_bitwarden "$aur_helper" "$dry_run"
    fi
