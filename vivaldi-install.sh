...
Install or update Vivaldi web browser on Manjaro Linux.
...
install_vivaldi() {
    ...
    if [[ "$dry_run" == "true" ]]; then
        ...
        log "Vivaldi installed successfully"
    fi
}
...
update_vivaldi() {
    ...
    log "Updating Vivaldi..."
    if [[ "$dry_run" == "true" ]]; then
        log "[DRY RUN] Would execute: $aur_helper -S --noconfirm $PACKAGE_NAME"
    else
        $aur_helper -S --noconfirm "$PACKAGE_NAME"
        log "Vivaldi updated successfully"
    fi
}
...
main() {
    ...
    log "Starting Vivaldi installation/update check..."
    ...
    if is_package_installed; then
        ...
        log "Vivaldi is already installed (version: $installed_version)"
        
        if [[ "$force" == "true" ]]; then
            log "Force flag specified, reinstalling..."
            update_vivaldi "$aur_helper" "$dry_run"
            return 0
        fi
        ...
        case $cmp_result in
            0)
                log "Vivaldi is up to date (version: $installed_version)"
                ;;
            1)
        ...
        log "Vivaldi is not installed"
        install_vivaldi "$aur_helper" "$dry_run"
    fi
